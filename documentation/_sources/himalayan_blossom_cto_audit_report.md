...
The architecture is well-structured (Firebase Auth + Firestore + Razorpay + Vercel serverless), but the *customer-facing purchase experience* has major gaps that will cause **high drop-off rates** and **zero repeat purchases**.
...
```html
<span data-en="250 gram">250 gram</span>  <!-- ← No price shown -->
...
**Sprint 2 (Retention — 2 weeks):**
5. Build "My Orders" page — list past orders from `users/{uid}/orders`
6. Add order status field (Processing / Shipped / Delivered) + admin update flow
7. Send confirmation email via Nodemailer/SendGrid via API route
8. Replace `window.prompt()` notes with inline input

**Sprint 3  (Growth — 2–3 weeks):**
9. Individual product detail pages
10. Guest checkout (collect email at checkout without full signup)
11. Coupon code support
12. Stock counter + "Notify me" for out-of-stock
